...
% X_data= X_data;
% %  centering 
MainCenter_o = median(X_data);
ERQ_o = X_data(((iw3>=4.6) & (rew>=2)),:);
ERQCenter_o = median(ERQ_o);
ERQVector_o = ERQCenter_o - MainCenter_o
...
isovalue = 0.025*0.65*max(max(max(D_mesh)));
surf = isosurface(xx,yy,zz, D_mesh, isovalue);
surfB4out = surf;
surfB4out.vertices=(surf.vertices - MainCenter)*1.5 + MainCenter;
...
mask1 = abs((surf_i.vertices(:,1) - P0(1))/ERQVector(1) - ...
             (surf_i.vertices(:,2) - P0(2))/ERQVector(2)) <0.005; 
mask2 = abs((surf_i.vertices(:,3) - P0(3))/ERQVector(3) - ...
            (surf_i.vertices(:,1) - P0(1))/ERQVector(1)) <0.005;

P = surf_i.vertices(mask1 & mask2,:)
